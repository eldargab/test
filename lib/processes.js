...
function* generator() {
  ...
  let lock = {}
  let lockerProcess = run(locker, lock)
  let spawnerProcess

  try {
    yield lock.ready
    spawnerProcess = run(spawner)
    while(running(lockerProcess) && running(spawnerProcess) && (yield lock.ready)) {
    ...
    redis.close()
    lockerProcess.abort()
    spawnerProcess && spawnerProcess.abort()
  }
}
  ...
  try {
    while(true) {
      ...
      while (expires < 300) {
        ...
        if (expires == 0) return props.ready.done(null, false)
      }
